...
import { runEngagementManual, engagementErrorCode } from "@/lib/services/engagement";
...
// POST /api/pipeline/engagement — run Stage 12 (the final stage). Fire-and-
// forget; on success the run is marked complete + a run_complete event fires.
export async function POST(request: NextRequest) {
  ...
  if (row.status === "running") {
    return errorJson(409, "STAGE_IN_PROGRESS", "Run is currently executing.");
  }
  if (row.titles_data === null || row.script_data === null) {
    return errorJson(409, "MISSING_PREREQUISITES", "Generate a title and script first.");
  }

  void (async () => {
    try {
      await runEngagementManual(row, user.id);
    } catch (err) {
      await publish(parsed.data.runId, {
        event: "run_error",
        payload: { runId: parsed.data.runId, stage: 12, code: engagementErrorCode(err) },
      });
    }
  })();
