...
import { abPlanErrorCode } from "@/lib/services/ab-plan";
...
// POST /api/pipeline/ab-plan — run Stage 11 (fire-and-forget, bus-streamed).
export async function POST(request: NextRequest) {
  ...
  if (row.titles_data === null || row.thumbnails_data === null) {
    return errorJson(409, "MISSING_PREREQUISITES", "Generate titles and thumbnails first.");
  }

  void (async () => {
    try {
      await runStage(parsed.data.runId, "ab", user.id);
    } catch (err) {
      await publish(parsed.data.runId, {
        event: "run_error",
        payload: { runId: parsed.data.runId, stage: 11, code: abPlanErrorCode(err) },
      });
    }
  })();
