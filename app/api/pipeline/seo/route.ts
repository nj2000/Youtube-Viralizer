...
import { seoErrorCode } from "@/lib/services/seo";
...
// POST /api/pipeline/seo — run Stage 10 (fire-and-forget, bus-streamed).
export async function POST(request: NextRequest) {
  ...
  if (row.titles_data === null || row.script_data === null) {
    return errorJson(409, "MISSING_PREREQUISITES", "Lock a title and generate a script first.");
  }

  void (async () => {
    try {
      await runStage(parsed.data.runId, "seo", user.id);
    } catch (err) {
      await publish(parsed.data.runId, {
        event: "run_error",
        payload: { runId: parsed.data.runId, stage: 10, code: seoErrorCode(err) },
      });
    }
  })();
