...
import { MissingHookPrereqError } from "@/lib/services/hook";
import { InvalidHookError } from "@/lib/services/hook-llm";
...
export async function POST(request: NextRequest) {
  ...
  void (async () => {
    try {
      await runStage(parsed.data.runId, "hook", user.id);
    } catch (err) {
      const code =
        err instanceof MissingHookPrereqError ||
        err instanceof MissingDependencyError
          ? "MISSING_PREREQUISITES"
          : err instanceof InvalidHookError
            ? "INVALID_HOOK"
            : "UPSTREAM_ERROR";
      await publish(parsed.data.runId, {
        event: "run_error",
        payload: { runId: parsed.data.runId, stage: 6, code },
      });
    }
  })();
