...
import {
  MissingHookPrereqError,
  regenerateHookVariant,
} from "@/lib/services/hook";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const next = await regenerateHookVariant({
    ...
    if (err instanceof InvalidHookError) {
      return errorJson(502, "INVALID_HOOK", "Hook generation failed.");
    }
    return errorJson(502, "UPSTREAM_ERROR", "Hook generation failed.");
  }
}
