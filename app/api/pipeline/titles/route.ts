...
import { MissingTitlePrereqError } from "@/lib/services/titles";
...
export async function POST(request: NextRequest) {
  ...
  void (async () => {
    try {
      await runStage(parsed.data.runId, "titles", user.id);
    } catch (err) {
      const code =
        err instanceof MissingTitlePrereqError ||
        err instanceof MissingDependencyError
          ? "MISSING_PREREQUISITES"
          : err instanceof CharLimitViolationError
            ? "CHAR_LIMIT_VIOLATION"
            : "UPSTREAM_ERROR";
      await publish(parsed.data.runId, {
        event: "run_error",
        payload: { runId: parsed.data.runId, stage: 5, code },
      });
    }
  })();
