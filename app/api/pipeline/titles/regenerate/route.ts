...
import {
  MissingTitlePrereqError,
  regenerateTrigger,
} from "@/lib/services/titles";
import { CharLimitViolationError } from "@/lib/services/titles-llm";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const next = await regenerateTrigger({
    ...
    return NextResponse.json({ titlesData: next }, { status: 200 });
  } catch (err) {
    if (err instanceof MissingTitlePrereqError) {
      return errorJson(409, "MISSING_PREREQUISITES", "Generate titles first.");
    }
    if (err instanceof CharLimitViolationError) {
      return errorJson(
        422,
        "CHAR_LIMIT_VIOLATION",
        "Couldn't keep the title under 100 characters.",
      );
    }
    return errorJson(502, "UPSTREAM_ERROR", "Title generation failed.");
  }
}
