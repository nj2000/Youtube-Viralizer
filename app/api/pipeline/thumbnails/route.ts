...
import { MissingThumbnailPrereqError } from "@/lib/services/thumbnails";
import { MissingDependencyError } from "@/lib/services/errors";
...
// POST /api/pipeline/thumbnails — run Stage 9 (fire-and-forget, bus-streamed).
export async function POST(request: NextRequest) {
  ...
  if (row.titles_data === null) {
    return errorJson(409, "MISSING_PREREQUISITES", "Lock a title first.");
  }

  void (async () => {
    try {
      await runStage(parsed.data.runId, "thumbnails", user.id);
    } catch (err) {
      const code =
        err instanceof MissingThumbnailPrereqError ||
        err instanceof MissingDependencyError
          ? "MISSING_PREREQUISITES"
          : "UPSTREAM_ERROR";
      await publish(parsed.data.runId, {
        event: "run_error",
        payload: { runId: parsed.data.runId, stage: 9, code },
      });
    }
  })();
