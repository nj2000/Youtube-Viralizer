...
import { isLintFresh } from "@/lib/services/lint-actions";
...
// POST /api/pipeline/lint/rerun — manual re-run. NO_CHANGES (409) when the
// stored lint already reflects current inputs and force !== true (spec §4.4).
export async function POST(request: NextRequest) {
  ...
  if (!parsed.data.force && isLintFresh(row)) {
    return errorJson(409, "NO_CHANGES", "Lint is already up to date.");
  }

  void (async () => {
    try {
      await runLintManual(row, user.id, { force: parsed.data.force });
    } catch (err) {
